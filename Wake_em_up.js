// Create a function called createAlarm that generates a wake-up message for a person 
// after a specified time delay. This function should accept two parameters: the person's 
// name and the delay time in seconds. The function should return a promise that resolves 
...
function createAlarm(name, delay) {
  return new Promise((resolve, reject) => {
    if(delay < 2){
      reject("Starting Delay is not sufficient");
    }else{
      setTimeout(()=> {
...
createAlarm("andrew", 1).then(res => console.log(res))
